...
def rubber_sheet(gray: np.ndarray,
    ...
    Classic rubber-sheet: map annulus [pupil, iris] to (radial_res x angular_res).
    Then:
      1) subtract local mean on ~16x16 blocks (exact for 64x360),
      2) histogram equalization on ~32x32 tiles (exact grid for 64x360).
    ...
    # subtract mean intensities of 16x16 blocks from image
    H, W = norm.shape
    bh = min(16, H) 
    bw = min(16, W) 
    ...
    # histogram equalization on 32x32
